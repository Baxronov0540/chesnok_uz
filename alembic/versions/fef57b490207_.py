...
def upgrade() -> None:
    ...
    op.add_column(
        "comments", sa.Column("created_at", sa.DateTime(timezone=True), nullable=True)
    )
    op.add_column(
    ...
    comments_table = table(
        "comments", column("created_at", DateTime), column("updated_at", DateTime)
    )

    op.execute(
        comments_table.update().values(created_at=func.now(), updated_at=func.now())
    )
