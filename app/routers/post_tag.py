...
from app.schemas import PostTagUpdateRequest,PostTagCreateRequest,PostTagListResponse
...
async def post_tag_create(session:db_deb,create_data:PostTagCreateRequest):
    if not create_data.post_id or not create_data.tag_id:
        return JSONResponse(status_code=200,content="{message: create_datada malumot yuq}")
    post_tag=PostTag(post_id=create_data.post_id,
