...
from app.schemas import ( PostCreateRequest,PostListResponse,PostUpdateRequest,
                         session_auth_dep,CategoryCreateRequest,
                         CategoryListResponse,CategoryUpdateRequest )
from app.models import Post,Category
from app.utils import generate_slug
router=APIRouter(prefix="/admin",tags=["Admin"])

@router.post("/post",response_model=PostListResponse)
async def post_create(db:db_deb,data:PostCreateRequest,current_user:session_auth_dep):
...
async def  get_post(session:db_deb,current_user:session_auth_dep):
    if not current_user.is_supperuser :
...
async def  post_delete(session:db_deb,post_id:int,current_user:session_auth_dep):
...
async def category_create(session:db_deb,current_user:session_auth_dep,data:CategoryCreateRequest):

    if not current_user.is_supperuser:
        raise HTTPException(status_code=403,detail="Sizda create qilish uchun   huquq yuq!!")
...
async def delete_category(session:db_deb,category_id:int,current_user:session_auth_dep):
    if not current_user.is_supperuser:
        raise HTTPException(status_code=403,detail="sizda huquq yuq!!")
...
async def  category_update(session:db_deb,update_data:CategoryUpdateRequest,current_user:session_auth_dep,category_id:int):
